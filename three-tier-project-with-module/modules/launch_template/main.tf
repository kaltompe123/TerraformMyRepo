data "aws_ami" "example" {
  ...
  filter {
    name   = "name"
    values = [var.frontend-ami]  # Use your AMI name pattern
  }
}
...
resource "aws_launch_template" "frontend" {
  ...
  vpc_security_group_ids = [var.frontendsg_id]
...
data "aws_ami" "example1" {
  ...
  filter {
    name   = "name"
    values = [var.backend-ami]  #use your AMI name pattern
  }
}
...
resource "aws_launch_template" "backend" {
  ...
  instance_type = "t2.micro"
  vpc_security_group_ids = [var.backendsg_id]
  #key_name = "project11thnov" 
  key_name = var.keyname
